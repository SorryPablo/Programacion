...
la Tab. \ref{tab:Potenciales vs. frecuencia} frente a la frecuencia. Realizamos también una regresión por mínimos cuadrados
...
Y por regresión de mínimos cuadrados:
...
\begin{center}
\begin{align}
    a = \frac{(\sum^n_{i=1} y_i)(\sum^n_{i=1} x^2_i )-(\sum^n_{i=1} x_i)(\sum^n_{i=1} x_i y_i)}{n(\sum^n_{i=1} x_i^2)-(\sum^n_{i=1} x_i)^2} &\Rightarrow a= 0,0581\hspace{6pt} [V] \label{eq:a4}\\
    b = \frac{n(\sum^n_{i=1} x_i y_i)-(\sum^n_{i=1} x_i)(\sum^n_{i=1} y_i)}{n(\sum^n_{i=1} x_i^2)-(\sum^n_{i=1} x_i)^2} &\Rightarrow b= 694880 \hspace{6pt}[\Omega] \label{eq:b4}\\
    s_{y|x} =\sqrt\frac{\sum^n_{i=1}(y_i-(a+b x_i))^2}{n-2} &\Rightarrow s_{y|x}= 0,0148 \hspace{6pt}[V] \label{eq:s4}\\
    s(a) =s_{y|x} \cdot \sqrt\frac{\sum^n_{i=1}x_i^2}{n(\sum^n_{i=1} x_i^2)-(\sum^n_{i=1} x_i)^2} &\Rightarrow s(a)= 0,0095 \hspace{6pt}[V] \label{eq:s4a}\\
    s(b) =s_{y|x} \cdot \sqrt\frac{n}{n(\sum^n_{i=1} x_i^2)-(\sum^n_{i=1} x_i)^2} &\Rightarrow s(b)= 970 \hspace{6pt}[\Omega] \label{eq:s4b}\\
    r = \frac{n(\sum^n_{i=1} x_i y_i)-(\sum^n_{i=1} x_i)(\sum^n_{i=1} y_i)}{\sqrt{[n(\sum^n_{i=1} x_i^2)-(\sum^n_{i=1} x_i)^2][n(\sum^n_{i=1} y_i^2)-(\sum^n_{i=1} y_i)^2]}}&\Rightarrow r= 0,999991 \label{eq:R4}\\
    R^2 =r^2&\Rightarrow R^2=0.99998 \label{eq:R24}
\end{align}
\end{center}
