...
\begin{equation}
\begin{split}
    \sum^n_{i=1} x_i = 30815 \hspace{3pt}[Hz] \hspace{20pt}
    \sum^n_{i=1} y_i = 21.65949972536621 \hspace{20pt}
    \sum^n_{i=1} x_i^2 = 88329719 \hspace{3pt} [{Hz}^2] \hspace{20pt} \\
    \sum^n_{i=1} x_i y_i = 60850.78570919058 \hspace{3pt} [Hz] \hspace{20pt}
...
\begin{center}
\begin{align}
    b = \frac{\sum^n_{i=1} x_i y_i}{\sum^n_{i=1} x_i^2} &\Rightarrow b= 0,000689 \hspace{6pt}[s] \label{eq:b4}\\
    s_{y|x} =\sqrt\frac{\sum^n_{i=1}(y_i-b x_i)^2}{n-1} &\Rightarrow s_{y|x}= 0,047411 \hspace{6pt} \label{eq:s4}\\
    s(b) =\frac{s_{y|x}}{\sqrt{\sum^n_{i=1} x_i^2}} &\Rightarrow s(b)= 0,000005 \hspace{6pt}[s] \label{eq:s4b}\\
    r = \frac{\sum^n_{i=1} x_i y_i}{\sqrt{(\sum^n_{i=1} x_i^2)(\sum^n_{i=1} y_i^2)}}&\Rightarrow r= 0,9994 \label{eq:R4}\\
    R^2 =r^2&\Rightarrow R^2=0.998 \label{eq:R24}
\end{align}
\end{center}
...
\begin{equation}
    \hat{y}=\hspace{3pt}(0,000689 \pm 0,000005)\mathscr{F} \hspace{6pt}
\end{equation}
...
dando como resultado puntos que están
indistinguiblemente sobre la recta. (CAMBIAR CUANDO SE SEPA ALGO DE LA RECTA)

Por otra parte, es claro que la pendiente resultante de minimizar el $ECM$ no coincide con el valor teórico,
pero tras un análisis más minucioso de la gráfica, distinguimos que todos los puntos quedan por encima de la recta, en otras palabras,
...
un error razonable que no saltaría de forma tan notable a la vista de hacer la regresión lineal con dos parámetros (perdiendo así 
un orden de aproximación de $r=0,9994\approx 1$). En cuanto a la calidad del ajuste, $R^2=0,998\approx 1$ es bueno y se concluye que
los datos siguen una regresión lineal. Mediante una estimación indirecta llegamos a que $$f_c\in\mathscr{F}_C\hspace{6pt} ,si\hspace{4pt} k=3 \iff 1426,29\in (1426,1558)\hspace{4pt}
[Hz]$$ lo cual no es muy bueno porque abarca un rango de más de $100\hspace{4pt} [Hz]$.
